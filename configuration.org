...
#+begin_src emacs-lisp
    (with-eval-after-load 'org
      ;; One global default sequence for all Org files:
      (setq org-todo-keywords
            '((sequence "TODO(t)" "NEXT(n)" "PROGRESS(r)" "WAITING(w)"
                        "|" "DONE(d)" "|" "POSTPONED(P)"))))
    #+end_src
...
(setq org-agenda-files (append (list 
                                ...
                                (concat dropbox-directory "projects.org")
                                )
                               ))
...
(defun my/org--uuid ()
  ...
  (cond
   ((fboundp 'org-id-uuid) (org-id-uuid))
   ((executable-find "uuidgen")
    (string-trim (shell-command-to-string "uuidgen")))
   (t
    (error "No UUID generator available; install uuidgen or load org-id"))))

(defun my/org--asset-month ()
  "Return current month abbreviation like Jan, Feb, etc."
  (format-time-string "%b"))

(defun my/org--asset-year ()
  "Return current year as a string."
  (format-time-string "%Y"))

(defun my/org--allowed-image-p (file)
  "Return non-nil if FILE has an allowed extension."
  (member (downcase (or (file-name-extension file) ""))
          '("jpg" "jpeg" "png" "pdf")))

(defun my/org--git-root (dir)
  "Return the git root for DIR, or nil if none."
  (locate-dominating-file dir ".git"))

(defun my/org--relative-or-absolute-path (target)
  "Return TARGET relative to current buffer file if possible, else absolute."
  (if buffer-file-name
      (file-relative-name target (file-name-directory buffer-file-name))
    (expand-file-name target)))

(defun my/org-add-external-file (file)
  "Copy FILE into ~/Dropbox/org/YEAR/MON/UUID_ORIGINAL_FILENAME.ext,
insert an Org link to it, then git-add and git-commit it."
  (interactive
   (list (read-file-name "Image/PDF: " nil nil t)))
  ...
  (unless (my/org--allowed-image-p file)
    (error "Only jpg, jpeg, png, and pdf files are supported"))

  (let* ((source (expand-file-name file))
         (ext    (downcase (file-name-extension source)))
         (base   (file-name-base source))
         (uuid   (my/org--uuid))
         (year   (my/org--asset-year))
         (mon    (my/org--asset-month))
         (dest-dir (expand-file-name
                    (format "~/Dropbox/org/%s/%s/" year mon)))
         (dest-name (format "%s_%s.%s" uuid base ext))
         (dest-file (expand-file-name dest-name dest-dir))
         (org-path (my/org--relative-or-absolute-path dest-file))
         (git-root (my/org--git-root dest-dir))
         (default-directory (or git-root default-directory)))

    (make-directory dest-dir t)
    (copy-file source dest-file nil)

    ;; Insert Org markup.
    ;; For images, add a caption-friendly block; for PDFs, insert a normal link.
    (cond
     ((member ext '("jpg" "jpeg" "png"))
      (insert (format "[[file:%s]]" org-path))
      (when (fboundp 'org-display-inline-images)
        (org-display-inline-images)))
     ((string= ext "pdf")
      (insert (format "[[file:%s]]" org-path))))

    ;; Save current Org buffer so the inserted link is written.
    (save-buffer)

    ;; Git add + commit if destination is inside a git repo.
    (if git-root
        (let* ((rel-to-repo (file-relative-name dest-file git-root))
               (commit-msg (format ":robot: Add org asset %s" dest-name)))
          (unless (zerop (call-process "git" nil nil nil "add" "--" rel-to-repo))
            (error "git add failed for %s" rel-to-repo))
          (unless (zerop (call-process "git" nil nil nil
                                       "commit" "-m" commit-msg "--" rel-to-repo))
            (error "git commit failed for %s" rel-to-repo))
          (message "Copied, inserted, and committed: %s" dest-file))
      (message "Copied and inserted: %s (not in a git repo, so no
    commit)" dest-file))))
...
10* Adding notes and Agenda at current file
...
#+begin_src emacs-lisp
(defun my/in-wsl-p ()
  "Return non-nil if we're running inside WSL."
  (and (eq system-type 'gnu/linux)
       (file-exists-p "/proc/sys/fs/binfmt_misc/WSLInterop")))

(defun my/wsl-windows-ankiconnect-ip ()
  "Get Windows host IP for the vEthernet (WSL) adapter by calling ipconfig.exe."
  (string-trim
   (shell-command-to-string
    "/mnt/c/Windows/System32/ipconfig.exe | tr -d '\\r' | \
     awk '/Ethernet adapter vEthernet \\(WSL\\)/{f=1} f && /IPv4 Address/{print $NF; exit}'")))

(setq org-anki-ankiconnnect-listen-address
      (if (my/in-wsl-p)
          (format "http://%s:8765" (my/wsl-windows-ankiconnect-ip))
        "http://127.0.0.1:8765"))
...
;;   (string-trim
;;    (shell-command-to-string
;;     "/mnt/c/Windows/System32/ipconfig.exe | tr -d '\\r' | grep -P 'Ethernet.*WSL' -A4 | cut -d\":\" -f2 | tail -n1 | sed -e 's/[[:space:]]*//g'")))
;; ;; org-anki uses this URL (default is http://127.0.0.1:8765 which fails in WSL2)
;; (setq org-anki-ankiconnnect-listen-address
;;       (format "http://%s:8765" (my/wsl-windows-ankiconnect-ip)))

(setq org-use-sub-superscripts '{})

(setq my/org-anki-default-deck "Import")

(defun my/org-anki-get-deck ()
  "Return deck from org header or fallback to default."
  (or (org-entry-get (point) "ANKI_DECK" t)
      my/org-anki-default-deck))

(defun my/org-anki-ensure-deck ()
  (unless (org-entry-get (point) "ANKI_DECK")
    (org-set-property "ANKI_DECK" my/org-anki-default-deck)))

(add-hook 'org-anki-before-entry-sync-hook #'my/org-anki-ensure-deck)

#+end_src


#+begin_src text
org-mode exports _ as subscripts and ^ as superscripts The default
export behavior can be customized using the menus Org -> Customize ->
Browse Org Group. To update the default behavior for subscripts &
superscripts, choose:
...
(defun my-org-auto-commit ()
  "Auto commit org files to git on save."
  (when (and buffer-file-name
             (string-match "\\.org\\'" buffer-file-name))
    (let* ((default-directory (locate-dominating-file buffer-file-name ".git"))
           (msg (format ":robot:  %s" (file-name-nondirectory buffer-file-name))))
...
* Pyright
This is used for learning about the Python code base
